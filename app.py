...
cm = {
    "MalayBERT": np.array([[1104,85,335],[90,757,81],[343,127,1054]]),
    "mBERT": np.array([[1119,72,333],[102,710,116],[378,96,1050]]),
    "XLM-R": np.array([[1165,55,304],[65,759,104],[283,95,1146]])
}

history = {
    "MalayBERT": [[1,.6063,.6128,.7375,.7364],[2,.4370,.6534,.7340,.7319],[3,.2980,.7769,.7413,.7415],[4,.1810,1.1485,.7325,.7327],[5,.0745,1.3578,.7372,.7371]],
    "mBERT": [[1,.8082,.6830,.7061,.7070],[2,.5927,.6432,.7190,.7181],[3,.4451,.6776,.7268,.7268]],
    "XLM-R": [[1,.8086,.6852,.7171,.7173],[2,.5662,.5821,.7674,.7663],[3,.4590,.5766,.7711,.7712]]
}

pages = ["Overview","ML Results","DL Results","Transformer Results",
         "Confusion Matrices","Training Curves","Literature Comparison",
...
# ---------------- TRANSFORMERS ----------------
    ...
    st.dataframe(tr.style.format({
        "Accuracy":"{:.2f}%","Precision":"{:.2f}%","Recall":"{:.2f}%",
        "F1":"{:.2f}%","Training Time (s)":"{:.2f}"
...
# ---------------- CONFUSION MATRICES ----------------
    ...
    st.title("🔲 Transformer Confusion Matrices")
    model = st.selectbox("Model",list(cm.keys()))
    labels = ["Negative","Neutral","Positive"]

    fig = px.imshow(cm[model],x=labels,y=labels,text_auto=True,
                    labels={"x":"Predicted","y":"Actual","color":"Count"},
                    title=f"{model} Confusion Matrix")
    st.plotly_chart(fig,use_container_width=True)

    norm = cm[model] / cm[model].sum(axis=1,keepdims=True)
    fig2 = px.imshow(norm,x=labels,y=labels,text_auto=".2f",
                     labels={"x":"Predicted","y":"Actual","color":"Proportion"},
                     title=f"{model} Normalised Confusion Matrix")
    st.plotly_chart(fig2,use_container_width=True)

# ---------------- TRAINING CURVES ----------------
elif page == "Training Curves":
    st.title("📈 Transformer Training Curves")
    model = st.selectbox("Model",list(history.keys()))
    h = pd.DataFrame(history[model],
                     columns=["Epoch","Train Loss","Val Loss","Accuracy","F1"])

    c1,c2 = st.columns(2)
    with c1:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=h.Epoch,y=h["Train Loss"],mode="lines+markers",name="Train Loss"))
        fig.add_trace(go.Scatter(x=h.Epoch,y=h["Val Loss"],mode="lines+markers",name="Validation Loss"))
        fig.update_layout(title="Loss",xaxis_title="Epoch",yaxis_title="Loss")
        st.plotly_chart(fig,use_container_width=True)
    with c2:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=h.Epoch,y=h.Accuracy*100,mode="lines+markers",name="Accuracy"))
        fig.add_trace(go.Scatter(x=h.Epoch,y=h.F1*100,mode="lines+markers",name="F1"))
        fig.update_layout(title="Validation Performance",xaxis_title="Epoch",yaxis_title="Score (%)")
        st.plotly_chart(fig,use_container_width=True)
